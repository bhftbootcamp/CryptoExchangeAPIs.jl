...
"""
    ...
    funding_rate(client::MexcClient = Mexc.Futures.public_client; kw...)
...
result = Mexc.Futures.funding_rate(;
    symbol = "BTC_USDT",
)
...
function funding_rate(client::MexcClient, query::FundingRateQuery)
    return APIsRequest{Data{FundingRateData}}("GET", "api/v1/contract/funding_rate/$(query.symbol)", query)(client)
end

function funding_rate(client::MexcClient = Mexc.Futures.public_client; kw...)
