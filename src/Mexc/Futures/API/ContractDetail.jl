...
"""
    ...
    contract_detail(client::MexcClient = Mexc.Futures.public_client; kw...)
...
result = Mexc.Futures.contract_detail()
...
function contract_detail(client::MexcClient, query::ContractDetailQuery)
    return if isnothing(query.symbol)
        APIsRequest{Data{Vector{ContractDetailData}}}("GET", "api/v1/contract/detail", query)(client)
    else
        APIsRequest{Data{ContractDetailData}}("GET", "api/v1/contract/detail", query)(client)
    end
end

function contract_detail(client::MexcClient = Mexc.Futures.public_client; kw...)
