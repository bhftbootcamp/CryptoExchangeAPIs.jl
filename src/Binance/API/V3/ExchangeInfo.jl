...
Base.@kwdef struct ExchangeInfoQuery <: BinancePublicQuery
    permissions::Maybe{Vector{Permission.T}} = nothing
...
function Serde.SerQuery.ser_type(::Type{<:ExchangeInfoQuery}, x::Vector{Permission.T})::String
    return "[\"" * join(x, "\",\"") * "\"]"
end
...
function exchange_info(client::BinanceClient = Binance.API.public_client; kw...)
